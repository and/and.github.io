---
layout: post
title: "Bringing Notational Velocity Back to Life on Modern macOS"
date: 2026-02-08
categories: []
issue_number: 4
---
...
<img width="582" height="660" alt="Image" src="https://github.com/user-attachments/assets/f4742aba-1db3-4caf-ad23-05040e7b6144" />
...
The build failed with three specific errors when compiling on Xcode 16 with macOS SDK 26.2:

### 1. Incompatible Function Pointer Types in LinkingEditor.m

The code attempted to dynamically swap cursor implementations for the I-beam cursor (to show a white cursor on dark backgrounds). It used runtime manipulation via `method_setImplementation()`, but assigned function pointers to `IMP` types without proper casting:

```objective-c
// Line 980 - This failed to compile
...
A similar issue where `methodForSelector:` returns an `IMP` (essentially a `void*` function pointer) but was being assigned to a typed function pointer without an explicit cast:

```objective-c
// Line 102 - This also failed
...
The Xcode project settings included `-whatsloaded` in `OTHER_LDFLAGS`, a legacy linker flag that was removed in modern versions of the macOS linker. The linker simply refused to link with an "unknown options" error.

## The Fix

The fixes were surgical and minimal — just enough to satisfy modern tooling without rewriting legacy code:
...
## How I Fixed It: Using Claude Opus 4.6

I approached this problem with [Claude Opus 4.6](https://www.anthropic.com/claude), Anthropic's latest AI model, running in the terminal via [Claude Code](https://docs.anthropic.com/en/docs/build-with-claude/claude-code).

The workflow looked like this:

1. **Clone and attempt to build** - Started with `xcodebuild` to see what failed
2. **Parse compiler errors** - Claude read the build output, identified the specific type mismatches and linker errors
3. **Fix in place** - Claude edited the source files directly, adding the necessary type casts
4. **Iterate** - Each build revealed the next error; Claude fixed them one by one
5. **Clean up** - Removed the deprecated linker flag from the Xcode project
6. **Package and release** - Created a DMG, set up the GitHub release, wrote documentation

The entire process—from "this doesn't build" to "here's a downloadable DMG with release notes"—took one morning session. Claude handled:
- Reading Objective-C code it had never seen before
- Understanding arcane Xcode build settings from 2006
- Making precise edits without breaking surrounding code
- Creating proper Git commits with co-authorship attribution
- Generating GitHub releases with detailed instructions

What impressed me most: Claude never suggested rewriting the entire thing in Swift or "modernizing" the architecture. It understood the goal was minimal intervention—just enough to build, nothing more. The AI's strength wasn't in replacing human judgment about what to preserve; it was in executing the tedious work of tracking down compiler errors and making type-safe fixes.

This is what AI-assisted development looks like in practice: not replacing developers, but handling the mechanical parts of software archaeology so you can focus on the decisions that matter.
...
### For Developers: Proper Code Signing

The real solution requires:
1. An Apple Developer account ($99/year)
2. Code signing with a Developer ID certificate
3. Notarization via Apple's automated malware scan

I've applied for a Developer account. Once approved, future releases will be properly signed and notarized, eliminating the security warning entirely.
